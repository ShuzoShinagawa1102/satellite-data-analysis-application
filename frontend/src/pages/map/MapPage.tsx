...
import { fetchSitesGeoJSON, searchSatelliteScenes } from "../../api/morido";
import type {
  SiteGeoFeature,
  SiteStatus,
...
export default function MapPage() {
  ...
  useEffect(() => {
    setSatLoading(true);
    searchSatelliteScenes({ auto_bbox: "true", limit: 10, max_cloud_cover: 30 })
  ...
  return (
    <Box sx={{ display: "flex", height: "calc(100vh - 115px)" }}>
      ...
      >
        ...
        {/* 衛星データパネル */}
        <Box sx={{ borderTop: 1, borderColor: "divider", p: 1.5, maxHeight: 260, overflow: "auto" }}>
          ...
          <Stack spacing={1}>
            {satellites.slice(0, 5).map((scene) => (
